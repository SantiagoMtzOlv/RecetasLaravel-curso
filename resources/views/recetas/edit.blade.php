...
@section('content')
    <h2 class="text-center mb-5">Editar Receta de {{ $receta->titulo }}</h2>
        ...
        <div class="col-md-8">
            <form action="{{ route('recetas.update', $receta->id)}}" method="post" novalidate enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select name="categoria" id="categoria" class="form-control @error('titulo') is-invalid @enderror">
                        ...
                        @foreach ($categorias as $categoria)
                            <option value="{{ $categoria->id }}" {{ $receta->categoria->id == $categoria->id ? 'selected' : ''}}>{{ $categoria->nombre }}</option>
                        @endforeach
                    </select>
                    @error('categoria')
                        <span class="invalid-feedback d-block" role="alert">
                ...
                <div class="form-group mt-4">
                    ...
                    <div class="mt-4">
                        <p>Imagen Actual</p>
                        <img src="/storage/{{ $receta->imagen }}" alt="{{ $receta->itulo }}" style="width: 300px;">
                    </div>
                    @error('imagen')
                        <span class="invalid-feedback d-block" role="alert">
                            <strong>{{ $message }}</strong>
